...
from mfg import opinion_lib, util
from mfg.constraint import Sampler
...
def base_drift_builder(opt) -> Optional[DriftFn]:
...
class BaseSDE(metaclass=abc.ABCMeta):
    def __init__(self, opt, p0: Sampler, pT: Sampler):
        self.opt = opt
...
class SimpleSDE(BaseSDE):
    def __init__(self, opt, p: Sampler, q: Sampler, base_drift: Optional[DriftFn] = None):
...
class OpinionSDE(SimpleSDE):
    ...
    def __init__(self, opt, p: Sampler, q: Sampler):
...
def build_sde(opt, p: Sampler, q: Sampler) -> SimpleSDE:
